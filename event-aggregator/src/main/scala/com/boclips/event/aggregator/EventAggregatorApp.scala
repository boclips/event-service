...
import com.boclips.event.aggregator.infrastructure.mongo.{MongoContentPackageLoader, MongoVideoLoader, SparkMongoClient}
import com.boclips.event.aggregator.infrastructure.videoservice.{ContentPackageMetricsClient, ContentPackageMetricsClientResult}
import com.boclips.event.aggregator.infrastructure.youtube.YouTubeService
import com.boclips.event.aggregator.presentation.assemblers.VideoTableRowAssembler
  ...
  def getVideoIdsForContentPackages(
    ...
    contentPackages.mapPartitions { chunk =>
      ...
      chunk.flatMap { contentPackage =>
        ...
        println(s"cursor sum: ${result.cursor.get.map(_.asDigit).sum}")
        var videoIds = result.videoIds
        var i = 0
        while (result.videoIds.nonEmpty) {
          println(s"[$i] ${contentPackage.name} (${contentPackage.id.value})")
...
class EventAggregatorApp(
  ...
  def run(): Unit = {
    ...
    val videoIdsForContentPackages = configuration
      .contentPackageMetrics.map( config =>
    ...
    println(videoIdsForContentPackages.count())
    val videosWithRelatedData = VideoTableRowAssembler.assembleVideosWithRelatedData(
      videos,
      session.sparkContext.emptyRDD, // playbacks,
      session.sparkContext.emptyRDD, // users,
      session.sparkContext.emptyRDD, // orders,
      session.sparkContext.emptyRDD, // channels,
      session.sparkContext.emptyRDD, // contractsWithRelatedData,
      session.sparkContext.emptyRDD, // collections,
      session.sparkContext.emptyRDD, // impressions,
      session.sparkContext.emptyRDD, // videoInteractions,
      session.sparkContext.emptyRDD, // youtubeStatsByVideoPlaybackId,
      contentPackages,
