...
class CollectionFormatterTest extends Test {
  ...
  it should "write collection title" in {
    ...
    val json = CollectionFormatter formatRow presentation.CollectionWithRelatedData(collection, List(), List())
  ...
  it should "write deletion flag" in {
    CollectionFormatter.formatRow(presentation.CollectionWithRelatedData(createCollection(deleted = true), List(), List())).getBool("deleted") shouldBe true
    CollectionFormatter.formatRow(presentation.CollectionWithRelatedData(createCollection(deleted = false), List(), List())).getBool("deleted") shouldBe false
  }

  it should "write public flag" in {
    CollectionFormatter.formatRow(presentation.CollectionWithRelatedData(createCollection(public = true), List(), List())).getBool("public") shouldBe true
    CollectionFormatter.formatRow(presentation.CollectionWithRelatedData(createCollection(public = false), List(), List())).getBool("public") shouldBe false
  }
  ...
  it should "write collection interactions" in {
    ...
    val json = CollectionFormatter formatRow presentation.CollectionWithRelatedData(collection, List(), List(interaction))
