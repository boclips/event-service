...
class QueryScorerTest extends IntegrationTest {
  ...
  it should "approximate scores using Beta distribution" in sparkTest { spark =>
    ...
    val searchWithPlayback = createSearch(
      request = createSearchRequest(userIdentity = createBoclipsUserIdentity("u1"), query = "HELLO"),
    ...
    val searchWithoutPlayback = createSearch(
      request = createSearchRequest(userIdentity = createBoclipsUserIdentity("u2"), query = "Hello"),
  ...
  it should "calculate scores within a time period" in sparkTest { spark =>
    ...
    val aprilSearch = createSearch(
      request = createSearchRequest(userIdentity = createBoclipsUserIdentity("u1"), query = "HELLO", timestamp = TestTimestamps.thisYearIn(Month.APRIL)),
      interactions = createSearchInteractions(videosPlayed = List())
    )
    val maySearch = createSearch(
      request = createSearchRequest(userIdentity = createBoclipsUserIdentity("u1"), query = "HELLO", timestamp = TestTimestamps.thisYearIn(Month.MAY)),
      interactions = createSearchInteractions(videosPlayed = List())
    )
