...
class OrderFormatterTest extends Test {
  ...
  it should "write delivery date" in {
      val json = OrderFormatter formatRow createOrder(deliveredAt = Some(ZonedDateTime.parse("2020-11-20T20:21:22Z")))

      json.getString("deliveredAt") shouldBe "2020-11-20T20:21:22Z"
    }
  ...
  it should "write order source" in {
  ...
  it should "write order status" in {
    val json = OrderFormatter formatRow createOrder(status = "QUO")

    json.getString("orderStatus") shouldBe "QUO"
  }
